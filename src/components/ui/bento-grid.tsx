...
const BentoCard = ({
  ...
  <div
    key={name}
    className={cn(
      "group relative col-span-3 flex flex-col justify-between overflow-hidden rounded-xl bg-background transform-gpu [box-shadow:0_-20px_80px_-20px_#ffffff1f_inset]",
      className
    )}
    {...props}
  >
    <div>{background}</div>
    {badge ? (
      <div className="pointer-events-none absolute top-3 right-3 z-10">{badge}</div>
    ) : null}
    <div className="p-4">
